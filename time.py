...
def fetch_node_traffic(row):
    ...
    # 🌟 关键修改 1：伪装成 Chrome 浏览器，防止被面板防火墙拦截
    headers = {
    ...
    try:
        # 🌟 关键修改 2：proxies={"http": None, "https": None} 强制不走任何代理，直连服务器！
        ...
        if data.get("success"):
            ...
            row['traffic_data'] = {'success': False, 'message': f'面板API报错: {data.get("msg", "未知错误")}',
                                   'users': []}
